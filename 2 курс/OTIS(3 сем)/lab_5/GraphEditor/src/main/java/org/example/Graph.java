...
public class Graph implements Serializable {
    ...
    private void findHamiltonianCyclesRecursive(GraphNode current, List<GraphNode> remainingNodes, List<String> currentPath, List<String> hamiltonianCycles) {
        if (remainingNodes.isEmpty()) {
            // Если остались только начальные вершины, и мы вернулись к начальной вершине, это Гамильтонов цикл.
    ...
    public static Graph tensorProduct(Graph graph1, Graph graph2) {
        Graph tensorProductGraph = new Graph(graph1.getName() + "TensorProduct" + graph2.getName());

        // Создаем вершины тензорного произведения как комбинации вершин исходных графов
        for (GraphNode node1 : graph1.getNodes()) {
            for (GraphNode node2 : graph2.getNodes()) {
                GraphNode tensorNode = new GraphNode(node1.getID() + " ⊗ " + node2.getID());
                tensorProductGraph.addNode(tensorNode.getID());
            }
        }

        // Создаем рёбра тензорного произведения
        for (GraphNode node1 : graph1.getNodes()) {
            for (GraphNode node2 : graph2.getNodes()) {
                for (GraphEdge edge1 : graph1.getEdges()) {
                    for (GraphEdge edge2 : graph2.getEdges()) {
                        if (edge1.getVertex1().equals(node1.getID()) && edge2.getVertex1().equals(node2.getID())) {
                            String sourceNodeID = node1.getID() + " ⊗ " + node2.getID();
                            String targetNodeID = tensorProductGraph.getNodeById(edge1.getVertex2() + " ⊗ " + edge2.getVertex2()).getID();
                            tensorProductGraph.addEdge(sourceNodeID, targetNodeID);
                        }
                    }
                }
            }
